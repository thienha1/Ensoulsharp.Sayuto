    ...
    class WallJump
    {
        static InventorySlot getItem
        {
            get
            {
                ...
                {
                    if (item.Id == ItemId.Warding_Totem || item.Id == ItemId.Greater_Vision_Totem_Trinket || item.Id == ItemId.Greater_Stealth_Totem_Trinket)
                    {
                        return item;
                    }
        ...
        internal static void OnDraw()
        {
            ...
            {
                Render.Circle.DrawCircle(point.From.ToVector2().ToVector3(), player.BoundingRadius, System.Drawing.Color.White);
            }
        }
        ...
        internal static void OnUpdate()
        {
            if (Jumping && jumpspot != null)
            {
                ...
                var minion = ObjectManager.Get<AIMinionClient>().Where(x => x.IsValid && x.Team == GameObjectTeam.Neutral && x.IsVisible).MinOrDefault(x => x.Distance(jumpspot.From));
        ...
        {
            if (args.Msg == (uint)WindowsMessages.LBUTTONDOWN && SpellSlot.E.IsReady())
            {
                var jumpoint = Jumpspots.Find(x => x.Contains(Game.CursorPosRaw));
                if (jumpoint != null)
                {
                    Jumping = true;
                    jumpspot = jumpoint;
