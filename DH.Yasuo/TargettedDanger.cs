    ...
    class TargettedDanger
    {
        ...
        public class SData
        {
            ...
            internal float delay
            {
                get
                {
                    return Helper.GetSliderFloat("Evade", "enabled." + spellName + ".delay") + Helper.GetSliderInt("Evade", "Evade.Delay");
                }
            }
            internal bool IsEnabled
            {
                get
                {
                    return Helper.GetBool("Evade", "enabled." + spellName);
                }
            }
        }
        ...
        public static void OnUpdate()
        {
            ...
            {
                if (Helper.TickCount - ls.StartTick >= ls.data.delay + Helper.GetSliderInt("Evade", "Evade.Delay"))
                {
                    if (ls.poly.PointInPolygon(Helper.Yasuo.Position.ToVector2()) == 1)
                    {
                        var pos = Helper.Yasuo.Position.Extend(ls.argss.Target.Position, 50);
        ...
        struct LittleStruct
        {
            ...
            public AIBaseClientProcessSpellCastEventArgs argss;
        ...
        internal static void SpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            try
            {
                if (!Helper.GetBool("Evade", "Evade.WTS") || sender.IsAlly || !SpellSlot.W.IsReady() || (!Helper.GetBool("Evade", "Evade.FOW") && !sender.IsVisible))
                {
                    return;
                }
                if (args.SData.Name.Equals("MissFortuneBulletTime"))
                {
                    ...
                    if (ssdata.IsEnabled)
                    {
                        var end = args.Start.ToVector2().Extend(args.End.ToVector2(), 1400);
                        Evade.Geometry.Rectangle rect = new Evade.Geometry.Rectangle(args.Start.ToVector2(), end, args.SData.LineWidth);
                        ...
                        EnsoulSharp.SDK.Utility.DelayAction.Add(3000, () => DetectedPolygons.Clear());
                    }
                }
                if (!args.Target.IsMe)
                {
                ...
                {
                    var castpos = Helper.Yasuo.Position.Extend(args.Start, dist);
                    EnsoulSharp.SDK.Utility.DelayAction.Add((int)sdata.delay, () => Helper.Spells[Helper.W].Cast(castpos));
                }
            }
            catch
            {
