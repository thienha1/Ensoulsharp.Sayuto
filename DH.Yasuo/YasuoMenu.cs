...
namespace DH.Yasuo
{
    internal static class YasuoMenu
    {
        ...
        public static void Init(Yasuo yas)
        {
            ...
            Config = new Menu("Yasuo", "DH.Yasuo", true);
        ...
        internal static AMenuComponent GetEvadeMenu()
        {
            return Config.Parent[Assembly.GetExecutingAssembly().GetName() + ".Evade"];
        }
        ...
        struct Combo
        {
            internal static void Attach(Menu menu)
            {
                ...
                var blacklist = new Menu("BlackList", "Ult Targets");

                foreach (var hero in GameObjects.EnemyHeroes)
                {
                    blacklist.AddBool("ult" + hero.CharacterName, "Ult " + hero.CharacterName);
                }

                ultmenu.Add(blacklist);
        ...
        struct Harass
        {
            internal static void Attach(Menu menu)
            {
                menu.AddKeyBind("Harass.KB", "Harass Key", System.Windows.Forms.Keys.H, KeyBindType.Toggle).Permashow(true, "Harass");
        ...
        struct Evade
        {
            internal static void Attach(Menu menu)
            {
                ...
                foreach (var spell in TargettedDanger.spellList.Where(x => GameObjects.EnemyHeroes.Any(e => e.CharacterName == x.championName)))
                {
                    Menu champmenu = (Menu)targettedmenu.Parent[spell.championName];
                ...
                foreach (var spell in TargettedDanger.spellList.Where(x => GameObjects.EnemyHeroes.Any(e => x.championName == "Baron")))
                {
                    Menu champmenu = (Menu)targettedmenu.Parent[spell.championName + " [Experimental] "];
                    champmenu.AddBool("enabled." + spell.spellName, spell.spellName, true);
                }

                menu.Add(targettedmenu);

                Menu Flee = new Menu("Flee Settings", "Flee");
                Flee.AddKeyBind("Flee.KB", "Flee Key", System.Windows.Forms.Keys.Z, KeyBindType.Press).Permashow(true, "Flee");
        ...
        struct Misc
        {
            internal static void Attach(Menu menu)
            {
                ...
                menu.AddKeyBind("Misc.TowerDive", "Tower Dive Key", System.Windows.Forms.Keys.T, KeyBindType.Press).Permashow(true, "Tower Diving Override");
                menu.AddSList("Hitchance.Q", "Q Hitchance", new[] { "Low", "Medium", "High", "VeryHigh" }, 2);
                ...
                menu.AddKeyBind("Misc.DashMode", "Move to Mouse", System.Windows.Forms.Keys.NumPad9, KeyBindType.Press);
        ...
        struct Drawings
        {
            internal static void Attach(Menu menu)
            {
                ...
                menu.AddBool("Drawing.DrawQ", "Draw Q");
                menu.AddBool("Drawing.DrawE", "Draw E");
                menu.AddBool("Drawing.DrawR", "Draw R");
